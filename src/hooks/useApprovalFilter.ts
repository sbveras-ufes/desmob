...
export const useApprovalFilter = (vehicles: ApprovalVehicle[], filters: ApprovalFilters) => {
  return useMemo(() => {
    return vehicles.filter((vehicle) => {
      if (filters.demobilizationCode && !vehicle.demobilizationCode?.toLowerCase().includes(filters.demobilizationCode.toLowerCase())) {
        return false;
      }
      ...
      if (filters.diretoria && filters.diretoria.length > 0 && !filters.diretoria.includes(vehicle.diretoria)) {
        return false;
      }
      if (filters.ufEmplacamento && vehicle.ufEmplacamento !== filters.ufEmplacamento) {
        return false;
      }
      if (filters.uf && vehicle.uf !== filters.uf) {
        return false;
      }
      if (filters.cnpjProprietario && filters.cnpjProprietario.length > 0 && !filters.cnpjProprietario.includes(vehicle.cnpjProprietario!)) {
        return false;
      }
      if (filters.empresaProprietaria && vehicle.empresaProprietaria !== filters.empresaProprietaria) {
        return false;
      }
      if (filters.situacao && vehicle.situacao !== filters.situacao) {
        return false;
      }
      if (filters.situacaoAnaliseFiscal && vehicle.situacaoAnaliseFiscal !== filters.situacaoAnaliseFiscal) {
        return false;
      }
