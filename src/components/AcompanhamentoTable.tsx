...
const AcompanhamentoTable: React.FC<AcompanhamentoTableProps> = ({ 
  ...
  const renderDefaultCells = (vehicle: ApprovalVehicle) => (
    <>
  ...
  const renderAssetManagementCells = (vehicle: ApprovalVehicle) => (
    <>
      <td className="px-2 py-2 text-sm font-medium text-gray-900">{vehicle.placa}</td>
      <td className="px-2 py-2 text-sm">
        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getSituacaoColor(vehicle.situacao)}`}>
        ...
        <table className="w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              {layout === 'default' ? renderDefaultHeaders() : renderAssetManagementHeaders()}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {vehicles.map((vehicle) => (
              <tr key={vehicle.id} className="hover:bg-gray-50">
                ...
                {layout === 'default' ? renderDefaultCells(vehicle) : renderAssetManagementCells(vehicle)}
